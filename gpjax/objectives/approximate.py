...
def elbo(
    ...
    def bound(
        params: dict, training: SparseDataset, static_params: dict = None
    ) -> Array:
        ...
        trace_term = 0.5 * jnp.clip(
            jnp.sum(jnp.diag(Kff) - jnp.diag(Qff)) / params["obs_noise"], a_min=0.0
        )
